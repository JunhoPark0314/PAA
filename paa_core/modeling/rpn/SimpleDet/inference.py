...
class SDPostProcessor(torch.nn.Module):
    def __init__(
        self,
        pre_nms_thresh,
        pre_nms_top_n,
        ...
        super(SDPostProcessor, self).__init__()
...
def make_sd_postprocessor(config, box_coder):

    return SDPostProcessor(
        pre_nms_thresh=config.MODEL.SD.INFERENCE_TH,
        pre_nms_top_n=config.MODEL.SD.PRE_NMS_TOP_N,
        nms_thresh=config.MODEL.SD.NMS_TH,
        fpn_post_nms_top_n=config.TEST.DETECTIONS_PER_IMG,
        min_size=0,
        num_classes=config.MODEL.SD.NUM_CLASSES,
        bbox_aug_enabled=config.TEST.BBOX_AUG.ENABLED,
        box_coder=box_coder,
        bbox_aug_vote=config.TEST.BBOX_AUG.VOTE,
        score_voting=config.MODEL.SD.INFERENCE_SCORE_VOTING,
    )