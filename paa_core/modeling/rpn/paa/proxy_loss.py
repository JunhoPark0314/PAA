...
class PAAProxyLossComputation(object):
    ...
    def prepare_iou_based_proxy_targets(self, pred_box, targets, anchors, proxy_target_in=None):
        ...
        for im_i in range(len(targets)):
            ...
            mask = matched_idxs_per_im >= 0
            matched_targets = targets_per_im[matched_idxs_per_im[mask]]
            gt_iou_per_im = torch.zeros_like(matched_idxs_per_im).float()
            pred_box_per_im = self.box_coder.decode(pred_box_per_im_list[im_i][mask], anchors_per_im.bbox[mask])

            gt_iou_per_im[mask] = self.compute_ious(matched_targets.bbox, pred_box_per_im)
...
def make_paa_proxy_loss_evaluator(cfg, box_coder):
    loss_evaluator = PAAProxyLossComputation(cfg, box_coder)
